...
static int lima_pdev_probe(struct platform_device *pdev)
{
	...
	err = lima_device_init(ldev, ddev);
	if (err) {
	...
	if (err < 0) {
		dev_err(&pdev->dev, "error during drm register\n");
		goto err2;
	}

	dev_info(&pdev->dev, "lima loaded successfully!\n");
...
static int lima_pdev_remove(struct platform_device *pdev)
{
	...
	struct lima_device *ldev = to_lima_dev(ddev);
